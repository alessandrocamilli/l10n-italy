...
class withholding_tax_move(orm.Model):
    ...
    _columns = {
            'wt_move_payment_id': fields.many2one('withholding.tax.move.payment', 
                                    'Move Payment', readonly=True,),
        }
...
class withholding_tax_move_payment(orm.Model):
    ...
    def generate_from_moves(self, cr, uid, move_ids, context=None):
        ...
        if wt_moves:
            # Create Move payment
            val ={
                'name': sequence_obj.get(cr, uid, 'withholding.tax.move.payment'),
                'date': datetime.today(),
    ...
    def create_account_move(self, cr, uid, ids, context=None):
        ...
        for mp in self.browse(cr, uid, ids):
            ...
            # Period
            period_ids = period_obj.find(cr, uid, mp.date_payment, context=None)
            ...
            for wt_move in mp.line_ids:
                ...
                move_line_id = account_move_line_obj.create(cr, uid, {
                    'name': _('Withholding Tax Payment %s') % wt_move.partner_id.name,
